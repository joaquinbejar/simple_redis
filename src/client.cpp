...
namespace simple_redis {
    ...
    RedisClient::RedisClient(RedisConfig &config) : m_config(config) {
        if (this->m_config.validate()) {
            this->m_config.logger.log_debug("RedisDB client created with valid config");
        } else {
            this->m_config.logger.log_error("RedisDB client created with invalid config");
    ...
    RedisClient::~RedisClient() {
        if (!this->m_config.is_moved()) {
            this->m_config.logger.log_debug("RedisDB client destroyed");
        }

    }

    RedisClient::RedisClient(const RedisClient &other) {
        m_config.logger.log_debug("RedisDB client copy constructor");
        if (this == &other) {
            m_config.logger.log_debug("This and other are the same");
            return;
        }
    ...
    void RedisClient::connect() {
        if (!this->m_config.validate()) {
            this->m_config.logger.log_error("RedisDB trying to connect with invalid config");
            return;
        }
        json json_config = this->m_config.to_json();
        this->m_config.logger.log_debug("Connecting to RedisDB");
        this->m_config.logger.log_debug("RedisDB password: " + json_config["password"].get<std::string>());
        this->m_config.logger.log_debug("RedisDB hostname: " + json_config["host"].get<std::string>());
        this->m_config.logger.log_debug("RedisDB port: " + std::to_string(json_config["port"].get<int>()));
        this->m_config.logger.log_debug("RedisDB tag: " + json_config["tag"].get<std::string>());
        this->m_config.logger.log_debug("RedisDB size: " + std::to_string(json_config["size"].get<int>()));
        this->m_config.logger.log_debug("RedisDB connect_timeout: " +
                                        std::to_string(json_config["connect_timeout"].get<int>()));
        this->m_config.logger.log_debug("RedisDB socket_timeout: " +
                                        std::to_string(json_config["socket_timeout"].get<int>()));
        this->m_config.logger.log_debug("RedisDB loglevel: " + *this->m_config.loglevel);
        this->m_config.logger.log_debug(
                "RedisDB keep_alive: " + std::to_string(json_config["keep_alive"].get<bool>()));
        this->m_config.logger.log_debug("RedisDB db: " + std::to_string(json_config["db"].get<int>()));
        this->m_config.logger.log_debug("RedisDB wait_timeout: " +
                                        std::to_string(json_config["wait_timeout"].get<int>()));
        this->m_config.logger.log_debug("RedisDB connection_lifetime: " +
                                        std::to_string(json_config["connection_lifetime"].get<int>()));
        this->m_config.logger.log_debug("RedisDB connection_idle_time: " +
                                        std::to_string(json_config["connection_idle_time"].get<int>()));
        this->m_config.logger.log_debug(
    ...
    bool RedisClient::is_connected() {
        if (m_server_write == nullptr) {
            this->m_config.logger.log_error("Write server is null");
            return false;
        } else if (m_server_read == nullptr) {
            this->m_config.logger.log_error("Read server is null");
            return false;
        } else {
            try {
                ...
                this->m_config.logger.log_error("RedisDB PING failed: " + std::string(e.what()));
                return false;
            }
        }
    }
    ...
    bool RedisClient::del(const std::string &key) {
        ...
        try {
            ...
            this->m_config.logger.log_error("RedisDB DEL failed: " + std::string(e.what()));
        }
        return false;
    }
    ...
    bool RedisClient::del(const std::vector<std::string> &keys) {
        ...
        try {
            ...
            this->m_config.logger.log_error("RedisDB std::exception DEL failed: " + std::string(e.what()));
        }
        return false;
    }
        ...
        try {
            ...
            while (std::getline(f, line)) {
                if (line.find("master") != std::string::npos) {
                    auto ip_port = simple_common::ip_and_port_from_string_to_pair(line);
                    result.push_back(ip_port);
                }
            }
            return result;
        } catch (const std::exception &e) {
            this->m_config.logger.log_error("RedisDB get_cluster_nodes_master failed: " + std::string(e.what()));
        }
        return result;
    }
    ...
    std::vector<std::pair<std::string, std::string>> RedisClient::get_cluster_nodes_slave() {
        ...
        try {
            ...
            this->m_config.logger.log_error("RedisDB get_cluster_nodes_slave failed: " + std::string(e.what()));
        }
        return result;
    }
    ...
    std::vector<sw::redis::Redis> RedisClient::get_masters_nodes() {
        ...
        for (auto &master: masters) {
            ...
            if (simple_common::is_ip(connection_options.host) &&
                simple_common::is_a_valid_port(connection_options.port))
    ...
    RedisClient::get_keys_in_node(const std::pair<std::string, std::string> &node, const std::string &pattern) {
        ...
        try {
            ...
            this->m_config.logger.log_error(
                    ...
                    typeid(r_e).name());
            this->m_config.logger.log_error("Error in node: " + node.first + ":" + node.second);

        } catch (const std::exception &e) {
            this->m_config.logger.log_error("RedisDB std::exception GET_KEYS_IN_NODE failed: " + std::string(e.what()));
        }
        return keys;
    }
    ...
    std::vector<std::string> RedisClient::keys(const std::string &pattern) {
        ...
        try {
            ...
            this->m_config.logger.log_error("RedisDB std::exception KEYS failed: " + std::string(e.what()));
        }
        return keys;
    }
